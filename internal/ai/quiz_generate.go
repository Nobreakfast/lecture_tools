...
文案简洁，课堂可直接使用。
如果教师提供了 PDF 课件文本，请优先围绕课件中的核心概念、定义、公式、流程、案例和容易混淆点出题；不要编造课件中没有依据的专有细节。
如果教师提供了 question_count，请尽量生成对应数量的题目。
如果教师提供了 quiz_id 或 quiz_title，请沿用到 YAML 顶层字段。
...
type QuizInitializeInput struct {
...
func (c *Client) InitializeQuiz(ctx context.Context, in QuizInitializeInput) (string, error) {
	userMsg, _ := json.Marshal(in)
	return c.chat(ctx, quizGenerateSystemPrompt, string(userMsg), 0.55)
}
