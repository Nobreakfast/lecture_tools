...
func (s *Server) pptDir() string {
	return filepath.Join(s.cfg.MetadataDir, "_admin")
}
...
func (s *Server) apiMaterials(w http.ResponseWriter, r *http.Request) {
	// Prefer course_id; fall back to legacy slug for old clients.
	if cidStr := r.URL.Query().Get("course_id"); cidStr != "" {
		cid, err := strconv.Atoi(cidStr)
		if err == nil && cid > 0 {
			course, err := s.store.GetCourse(r.Context(), cid)
			if err == nil && course != nil {
				dir := s.metadataMaterialsDir(course.TeacherID, course.Slug)
				items, _ := s.scanMaterialsFromDir(dir, course.Slug, false)
				if items == nil {
					items = []adminMaterialGroupItem{}
				}
				publicItems := make([]materialGroupItem, 0, len(items))
				for _, item := range items {
					gi := materialGroupItem{Folder: item.Folder, Stem: item.Stem}
					if item.PDF != nil && item.PDF.Visible {
						gi.PDF = &materialPreviewItem{File: item.PDF.File, PreviewURL: item.PDF.PreviewURL, DownloadURL: item.PDF.DownloadURL, Size: item.PDF.Size}
					}
					for _, d := range item.Downloads {
						if d.Visible {
							gi.Downloads = append(gi.Downloads, materialDownloadItem{File: d.File, Extension: d.Extension, URL: d.URL, Size: d.Size})
						}
					}
					if gi.PDF != nil || len(gi.Downloads) > 0 {
						publicItems = append(publicItems, gi)
					}
				}
				writeJSON(w, map[string]any{"items": publicItems})
				return
			}
		}
	}
	courseSlug := strings.TrimSpace(r.URL.Query().Get("course"))
...
func (s *Server) scanMaterialsFromDir(dir, folder string, includeHidden bool) ([]adminMaterialGroupItem, error) {
	visibility, err := s.loadMaterialVisibility(contextOrBackground(nil))
	if err != nil {
		return nil, err
	}
	...
	sort.Slice(items, func(i, j int) bool { return items[i].Stem < items[j].Stem })
	return items, nil
}

func (s *Server) resolveMaterialFilePath(slug, fileName string) string {
	pattern := filepath.Join(s.cfg.MetadataDir, "*", safePathPart(slug), "materials", fileName)
	if matches, _ := filepath.Glob(pattern); len(matches) > 0 {
		return matches[0]
	}
	return filepath.Join(s.pptDir(), slug, fileName)
}
